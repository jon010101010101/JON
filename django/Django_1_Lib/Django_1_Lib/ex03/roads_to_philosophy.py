...
def get_first_valid_link(soup):
    """
    Gets the first valid link from a Wikipedia page.
...
def roads_to_philosophy(start_term):
    """
    Finds the path from an initial term to the Wikipedia page 'Philosophy'.

    Args:
    - start_term (str): Initial term from where the search begins.

    Returns:
    - None: Prints the path traversed and the number of steps to reach 'Philosophy',
...
if __name__ == "__main__":
    ...
    calls the `roads_to_philosophy` function with the provided initial term.
    
    Usage:
      python3 roads_to_philosophy.py "initial term"
