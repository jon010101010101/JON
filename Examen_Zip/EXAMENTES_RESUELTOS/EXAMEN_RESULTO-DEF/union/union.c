...
int main(int argc, char **argv)
{
    ...
    char seen[256] = {0};  // Arreglo para marcar caracteres ya vistos // tamaño max array 255 + 1 incluya cero
    int i = 0;
    int j = 0;

    // Procesar la primera cadena
    while (argv[1][i])
    {
        if (!seen[(unsigned char)argv[1][i]])
        {  // Si el carácter no ha sido visto
            seen[(unsigned char)argv[1][i]] = 1; // Marcar el carácter como visto
            write(1, &argv[1][i], 1);            // Mostrar el carácter
        }
        i++;
    }

    // Procesar la segunda cadena
    while (argv[2][j])
    {
        if (!seen[(unsigned char)argv[2][j]])
        {  // Si el carácter no ha sido visto
            seen[(unsigned char)argv[2][j]] = 1; // Marcar el carácter como visto
            write(1, &argv[2][j], 1);            // Mostrar el carácter
        }
        j++;
    }

    write(1, "\n", 1);  // Imprimir el salto de línea al final
    return 0;
}
