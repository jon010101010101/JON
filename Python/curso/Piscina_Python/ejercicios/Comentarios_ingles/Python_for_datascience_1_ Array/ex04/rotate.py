...
def manual_transpose(image: np.ndarray) -> np.ndarray:
    ...
    """
    #extrae la altuna y el ancho 
    height, width = image.shape[:2]
    # crea un nuevo NUMpy se invierten (widht, heigt) en luagr de (heigt, width)
    transposed_image = np.zeros((width, height), dtype=image.dtype)
    #itera sobre cada pixel y a cada pixel en su posicion traspuesta y se 
    # inverte los indices [j, i`en lugar de [i, j]`]
